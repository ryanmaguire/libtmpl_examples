...
#define NRoots ((sizeof(coeffs) / sizeof(coeffs[0])) - 1)
...
int main(void)
{
    ...
    if (norm == 0.0)
    {
        puts("Your input camera_pos is the zero vector. Aborting.\n");
        return -1;
    }
        ...
        for (x=0; x<size; ++x)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    min = tmpl_CDouble_Abs(
                        tmpl_CDouble_Subtract(root, roots[0])
                    );
                    ind = 0;
