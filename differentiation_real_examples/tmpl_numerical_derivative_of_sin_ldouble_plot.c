...
static void write_val(FILE * const fp, long double x, long double y)
{
    const double xin = (double)x;
    const double yin = (double)y;
    fwrite(&xin, sizeof(xin), 1, fp);
    fwrite(&yin, sizeof(yin), 1, fp);
}
...
int main(void)
{
    ...
    const long double start = -20.0L;

    /*  Right-most point on the x axis that is to be plotted.                 */
    const long double end = 20.0L;
    ...
    const long double dx = (end - start) / (long double)n_elements;
    ...
    for (n = zero; n < n_elements; ++n)
    {
        ...
        const long double y = tmpl_LDouble_Sin(x);
        const long double dy = tmpl_LDouble_Derivative(tmpl_LDouble_Sin, x);
        const long double z = tmpl_LDouble_Cos(x);
        const long double diff = tmpl_LDouble_Abs(z - dy);
