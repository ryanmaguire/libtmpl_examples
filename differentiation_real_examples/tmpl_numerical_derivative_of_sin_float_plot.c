...
static void write_val(FILE * const fp, float x, float y)
{
...
int main(void)
{
    ...
    const float dx = (end - start) / (float)n_elements;
    ...
    for (n = zero; n < n_elements; ++n)
    {
        ...
        const float y = tmpl_Float_Sin(x);
        const float dy = tmpl_Float_Derivative(tmpl_Float_Sin, x);
        const float z = tmpl_Float_Cos(x);
        const float diff = tmpl_Float_Abs(z - dy);
