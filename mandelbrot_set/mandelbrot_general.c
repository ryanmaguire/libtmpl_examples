...
static const double coeffs[4] = {1.0, 1.0, 1.0, 0.0};
static const size_t deg = sizeof(coeffs) / sizeof(coeffs[0]) - 1;

static tmpl_ComplexDouble
poly(tmpl_ComplexDouble z, tmpl_ComplexDouble c)
{
    tmpl_ComplexDouble out = tmpl_CDouble_Poly_Eval_Real_Coeffs(coeffs, deg, z);
...
int main(void)
{
    ...
    fp = fopen("mandelbrot_general.ppm", "w");
    fprintf(fp, "P6\n%d %d\n255\n", size, size);
    ...
    for (y = 0U; y < size; ++y)
    {
        ...
        z_y = (size - y - 1U) * (y_max - y_min) * rcp_factor + y_min;
